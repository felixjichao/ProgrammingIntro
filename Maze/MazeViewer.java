...
public class MazeViewer {
    ...
    private static MazeFrame readMazeFile(String fileName) throws IOException {
        ...
        boolean[][] mazeInput = null;
        MazeCoord startInput = null;
        MazeCoord endInput = null;

        while(in.hasNext()) {
            int rowNum = in.nextInt(); // read in the row number of the maze
            int colNum = in.nextInt(); // read in the column number of the maze

            // read in the distribution of the walls in the maze
            // stored as true if there is a wall; stored as false otherwise
            mazeInput = new boolean[rowNum][colNum];
            in.nextLine();
            for(int i = 0; i < rowNum; i++) {
                String current = in.nextLine();
                for(int j = 0; j < colNum; j++) {
                    if(current.charAt(j) == '1') { mazeInput[i][j] = true; }
                    else { mazeInput[i][j] = false; }
                }
            }

            // read in the entry location coordinates
            startInput = new MazeCoord(in.nextInt(), in.nextInt());

            // read in the exit location coordinates
            endInput = new MazeCoord(in.nextInt(), in.nextInt());
        }

        return new MazeFrame(mazeInput, startInput, endInput);
