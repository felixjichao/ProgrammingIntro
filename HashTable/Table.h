...
// add private data and methods here
    ...
    ListType *data;             // an dynamic array of pointers to ListType
    int entryNum;               // the number of entries
    int nonEmptyBucketNum;      // the number of non-empty bucket
    int * chainLength;          // an dynamic array of pointers to integers
                                // for the length of every chain in the table

    // calculate and return the length of the longest chain in the table
    int longestChain(int* target) const;
};
